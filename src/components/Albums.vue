   ...
   <svg aria-hidden="true" style="position: absolute; width: 0; height: 0; overflow: hidden;" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
    ...
    <div class="row">
      ...
      <div class="col-md-6 text-left mt-3">
        ...
        <p> 
          <label>{{ $t('sorting_methods') }}: <em class="sort">{{this.sort.names}} <svg :class="'icon icon-arrow-'+this.sort.direction"><use :xlink:href="'#icon-arrow-'+this.sort.direction"></use></svg></em></label><br/>
       
          <button @click="sortOnClick" type="button" name="author" class="btn btn-light mt-1 mr-1">{{ $t('author_text') }}</button>
          <button @click="sortOnClick" type="button" name="photos" class="btn btn-light  mt-1 mr-1">{{ $t('no_photos_text') }}</button>
          <button @click="sortOnClick" type="button" name="id" class="btn btn-light  mt-1 mr-1">{{ $t('id_album_text') }}</button>
        </p>
        
      </div>  
    </div>
    <div class="row">
      <clazy-load class="wrapper col caption_hover" v-for="(img, index) in images" :ratio=0.1 :src="'https://unsplash.it/500?image='+img.photos[0].id" :key="index">
        <transition name="fadeInUp">
          <figure>
            <router-link :to="{path:'album/' + img.author}" class="a">
              <img :src="'https://unsplash.it/500?image='+img.photos[0].id" width="300px" @mouseover="mouseOver" @mousemove="mouseOver" @mouseleave="mouseOut">
              <figcaption>
                <h3 :data-hover="'Album ' + img.id">Album {{img.id}}</h3>
                <span>{{ $t('author_text') }}: {{img.author}}</span><br/>
                <span>{{ $tc('photos',img.photos.length,{ count: img.photos.length}) }}
...
const computed = {
  getAlbumList() {
    this.sort.options.author = this.$i18n.messages[this.$i18n.locale].author_text;
    this.sort.options.id = this.$i18n.messages[this.$i18n.locale].id_album_text;
    this.sort.options.photos = this.$i18n.messages[this.$i18n.locale].no_photos_text;
...
.caption_hover {
  ...
  & h3 {
    ...
    text-fill-color: rgba($dark_color, 0.3);
    position: relative;
    font-size: 3.3em;
